...
for (sub in unique_subs) {
  ...
  name = paste('https://dayr8ngd7k1ny.cloudfront.net/', sub, sep = "")
...
df = cbind(unique_subs,url)
colnames(df) <- c('Suburb','Url')
df = df[order(as.character(df$Suburb)),]
df = df[-1,]
write.csv(as.matrix(df),'suburb_url.csv', row.names = FALSE)
a
